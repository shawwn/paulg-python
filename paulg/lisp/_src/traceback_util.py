...
# from paulg.lisp.lib import xla_extension
...
_lisp_message_append = (
    'The stack trace below excludes Lisp-internal frames.\n'
...
def filtering_mode():
    mode = paulg.lisp.config.lisp_traceback_filtering
...
def api_boundary(fun):
    ...
    stack trace of the original exception, but with Lisp-internal frames removed.
    ...
    This annotation is primarily useful in wrapping functions output by Lisp's
    transformations. For example, consider ``g = lisp.jit(f)``. When ``g`` is
    called, Lisp's JIT compilation machinery is invoked, which in turn calls ``f``
    in order to trace and translate it. If the function ``f`` raises an exception,
    the stack unwinds through Lisp's JIT internals up to the original call site of
    ``g``. Because the function returned by ``lisp.jit`` is annotated as an
    ``api_boundary``, such an exception is accompanied by an additional traceback
    that excludes the frames specific to Lisp's implementation.
    '''
