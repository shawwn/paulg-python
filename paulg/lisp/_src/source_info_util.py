...
_exclude_paths = [os.path.dirname(paulg.lisp.version.__file__)]
...
def user_frames(source_info: Optional[Traceback]) -> Iterator[Frame]:
  ...
  # Guess the user's frame is the innermost frame not in the lisp source tree
...
def panic(msg='Not implemented'):
  assert False, msg

def current() -> Optional[Traceback]:
  return _source_info_context.context or panic() #or xla_client.Traceback.get_traceback()

class LispStackTraceBeforeTransformation(Exception): pass

_message = (
    'The preceding stack trace is the source of the Lisp operation that, once '
    'transformed by Lisp, triggered the following exception.\n'
    '\n--------------------')

def has_user_context(e):
  while e is not None:
    if isinstance(e, LispStackTraceBeforeTransformation):
      return True
    e = e.__cause__
  return False
...
def user_context(c):
  ...
  try:
    ...
    if filtered_tb:
      ...
      msg = f'{msg}\n\n{_message}'
      c = LispStackTraceBeforeTransformation(msg).with_traceback(filtered_tb)
